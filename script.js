...
const driverImpactMap = {
  market_flow: 0,
  etf_adoption: 10,
  rate_cut: 8,
  rate_hike: -10,
  regulation_crackdown: -15,
  crypto_hack: -20,
  war_escalation: -12,
  neutral_macro: 0
};

const timeframeWeightMap = {
  "1m": 5,
  "5m": 3,
  "15m": 2,
  "1h": 0,
...
const macroNarratives = {
  market_flow: "Price action is driving the emotional state of the global crypto market.",
  etf_adoption: "Institutional flows and ETF headlines are boosting confidence across crypto.",
  rate_cut: "Liquidity expectations are improving market appetite for risk assets.",
  rate_hike: "Macro tightening pressure is making crypto sentiment more defensive.",
  regulation_crackdown: "Regulatory pressure is weighing on market confidence.",
  crypto_hack: "Security fears and insolvency risk are dragging sentiment lower.",
  war_escalation: "Geopolitical stress is creating uncertainty across the market.",
  neutral_macro: "No dominant macro event is strongly shifting crypto sentiment."
};
...
function applyMoodColor(element, moodKey) {
  if (!element) return;
  ...
  element.classList.add(`mood-${moodKey}`);
}
...
function updateHeroMoodVisual(score, mood) {
  ...
  if (heroScore) {
    ...
    applyMoodColor(heroScore, mood.key);
  }
...
function updateEmotionBar(score, mood) {
  ...
  if (byId("emotionBarMood")) {
    byId("emotionBarMood").textContent = mood.name;
    applyMoodColor(byId("emotionBarMood"), mood.key);
  }

  if (byId("emotionBarScore")) {
    byId("emotionBarScore").textContent = score;
    applyMoodColor(byId("emotionBarScore"), mood.key);
  }

  if (byId("emotionBarRange")) {
    byId("emotionBarRange").textContent = mood.range;
  }
}
...
function updateSocialMoodFromScore(score) {
  ...
  if (byId("socialMood")) {
    byId("socialMood").textContent = socialMood.name;
    applyMoodColor(byId("socialMood"), socialMood.key);
  }

  if (byId("socialScore")) {
    byId("socialScore").textContent = socialScore;
    applyMoodColor(byId("socialScore"), socialMood.key);
  }
...
function renderScale() {
  ...
  [...moods].reverse().forEach(mood => {
    ...
    item.innerHTML = `
      ...
      <strong class="mood-${mood.key}">${mood.name}</strong>
    `;
...
async function loadGlobalMarket() {
  ...
  const baseScore = scoreFromChange(marketCapChange);

  const selectedDriver = byId("macroDriver")?.value || "market_flow";
  const driverImpact = driverImpactMap[selectedDriver] || 0;
  const timeframeImpact = timeframeWeightMap[globalTimeframe] || 0;

  const finalScore = clamp(baseScore + driverImpact + timeframeImpact, 0, 100);
  const mood = getMoodByScore(finalScore);
  currentGlobalMood = mood;

  updateHeroMoodVisual(finalScore, mood);
  updateEmotionBar(finalScore, mood);

  updateSocialMoodFromScore(finalScore);

  if (byId("driverTechnical")) {
    byId("driverTechnical").textContent = mood.name;
    applyMoodColor(byId("driverTechnical"), mood.key);
  }

  if (byId("driverMacro") && byId("macroDriver")) {
    byId("driverMacro").textContent = byId("macroDriver").selectedOptions[0].textContent;
  }

  if (byId("driverNarrative")) {
    byId("driverNarrative").textContent = macroNarratives[selectedDriver];
  }
      ...
      <div class="coin-main">
        <div class="coin-topline">
          <div>
            ...
            <div class="coin-symbol mood-${mood.key}">${symbol}</div>
          </div>
        </div>

        <div class="price">${formatCurrency(coin.current_price)}</div>
        <div class="change mood-${mood.key}">${formatPercent(change)}</div>
      </div>

      <div class="coin-reaction">
        <img src="${getIconImagePath(style, mood.key)}" alt="${coin.name} mood">
      </div>
    `;
...
function updateCoinIntervalBoxes(coin) {
  ...
  Object.entries(mapping).forEach(([tf, id]) => {
    ...
    const mood = getMoodFromChange(value);
    const el = byId(id);
    if (!el) return;
    el.textContent = formatPercent(value);
    applyMoodColor(el, mood.key);
  });
}
...
async function loadCoinDetails() {
  ...
  if (byId("chartTitle")) {
    byId("chartTitle").textContent = `${coin.name} (${activeCoinSymbol})`;
  }

  if (byId("chartCoinLogo")) {
    byId("chartCoinLogo").src = coin.image;
    byId("chartCoinLogo").alt = `${coin.name} logo`;
  }

  if (byId("chartCoinPrice")) {
    byId("chartCoinPrice").textContent = formatCurrency(coin.current_price);
  }

  if (byId("coinMoodTitle")) byId("coinMoodTitle").textContent = `${coin.name} Mood`;
  if (byId("coinSocialMoodTitle")) byId("coinSocialMoodTitle").textContent = `${coin.name} Social Mood`;
  ...
  if (byId("selectedPerformance")) {
    ...
    applyMoodColor(byId("selectedPerformance"), mood.key);
  }

  if (byId("coinMoodLabel")) {
    byId("coinMoodLabel").textContent = mood.name;
    applyMoodColor(byId("coinMoodLabel"), mood.key);
  }

  if (byId("coinMoodScore")) {
    byId("coinMoodScore").textContent = score;
    applyMoodColor(byId("coinMoodScore"), mood.key);
  }

  if (byId("detailSocialLabel")) {
    byId("detailSocialLabel").textContent = socialMood.name;
    applyMoodColor(byId("detailSocialLabel"), socialMood.key);
  }

  if (byId("detailSocialScore")) {
    byId("detailSocialScore").textContent = socialScore;
    applyMoodColor(byId("detailSocialScore"), socialMood.key);
  }
...
Macro Driver: ${byId("driverMacro")?.textContent || "Market flow / price action"}
...
- Selected market driver: ${byId("driverMacro")?.textContent || "Market flow / price action"}
